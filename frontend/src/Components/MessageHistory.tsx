...
export const MessageHistory = () => {
	...
	useEffect(() => {
		...
		getAllMessages().catch((error) => console.error("Failed to get messages", error));
	}, [auth.userId]);
	...
	const MessageListItem: React.FC<MessageListItemProps> = ({ message, index, direction }) => (
		<li key={index} className="message-list-history">
			<div>
				<p className="text-lg">{message[0]}</p>
				<p className="m-2 text-sm text-neutral-content px-8">
					{direction}: {message[1].name}
				</p>
			</div>
